...
# initialize model
model = BiLSTMModel(vocab_size=len(vocab), embedding_dim=128, hidden_dim=256)

# create paths
model_path = models_dir / "bilstm_classifier.pt"
vocab_path = vocabs_dir / "bilstm_vocab.json"
...
if model_path.exists() and vocab_path.exists():
    ...
    logger = CSVLogger("ML/lightning_logs", name="bilstm")
...
test_preds.to_csv(preds_dir / "bilstm_test_preds.csv", index=False)
...
print("First 20 misclassified rows (full list saved to ML/artifacts/bilstm_misclassified.csv):")
print(misclassified.head(20).to_string(index=False))

# save full errors for inspection
misclassified.to_csv("ML/artifacts/bilstm_misclassified.csv", index=False)