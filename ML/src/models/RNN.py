...
# initialize model
model = RNNModel(vocab_size=len(vocab), embedding_dim=128, hidden_dim=256)

# create paths
model_path = models_dir / "rnn_classifier.pt"
vocab_path = vocabs_dir / "rnn_vocab.json"
...
if model_path.exists() and vocab_path.exists():
    ...
    logger = CSVLogger("ML/lightning_logs", name="rnn")
...
test_preds.to_csv(preds_dir / "rnn_test_preds.csv", index=False)
...
print("First 20 misclassified rows (full list saved to ML/artifacts/rnn_misclassified.csv):")
print(misclassified.head(20).to_string(index=False))

# save full errors for inspection
misclassified.to_csv("ML/artifacts/rnn_misclassified.csv", index=False)