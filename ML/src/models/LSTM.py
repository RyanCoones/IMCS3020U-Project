...
vocab_path = vocabs_dir / "lstm_vocab.json"
...
if model_path.exists() and vocab_path.exists():
    ...
    early_stopping = pl.callbacks.EarlyStopping(monitor="val_loss", stopping_threshold=0.045, mode="min")
    logger = CSVLogger("ML/lightning_logs", name="lstm")
...
# containers for predictions, true labels, probabilities and row indices
all_preds = []
all_true = []
all_probs = []
all_indices = []
...
# loop over test data
with torch.no_grad():
    for x, y, idx_batch in test_loader:
        x = x.to(device)
...
# simple error analysis: misclassified rows
...
print("First 20 misclassified rows (full list saved to ML/artifacts/lstm_misclassified.csv):")
print(misclassified.head(20).to_string(index=False))

# save full errors for inspection
misclassified.to_csv("ML/artifacts/lstm_misclassified.csv", index=False)